...
$connName = isset($_GET['conn']) ? $_GET['conn'] : $CommunityName;

if (!isset($connectionPool) || $connectionPool == null) {
    $connectionPool[$connName] = array('priority' => 10, 'connection' =>  $conn, 'master' => false, 'primary' => true);
}
...
foreach ($connectionPool as $name => $connDetails) {
    ...
    if ($connDetails['primary'] === true) {
        $primaryConn = $connDetails['connection'];
    }
}
...
<?php
$meetingBoxCount = 0;
$query = "SELECT count(*) as meetingBoxCnt FROM `meetingbox` WHERE `digitaldisplay` = 'Y'";
foreach ($connectionPool as $connName => $configuration) {
    ...
    while ($data = $resultARRAY->fetch_array(MYSQLI_ASSOC)) {
        $meetingBoxCount += $data['meetingBoxCnt'];
    }
}
?>

                    <div class="time-weather-container <?php if ($meetingBoxCount == '0'){ ?>no-newsboard<?php }; ?>">
<!-- Newsboard Banner Check -->
...
<?php include('../my-documents/owl.carousel-CLASSIC.php'); ?>
