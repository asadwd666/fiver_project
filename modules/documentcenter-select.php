...
?>
	    <tr>
	      <td align="left"><a href="<?php echo "{$url}";?>" <?php if ($row['window'] !== '') { ?><?php echo "{$row['window']}"; ?><?php }; ?> class="<?php if ($row['window'] == '') { ?>iframe-link<?php }; ?>" onclick="javascript:pageTracker._trackPageview('ColumnLink/<?php echo "{$row['title']}"; ?>'); "><?php echo "{$row['image']}"; ?></a>&nbsp;&nbsp;<a href="<?php echo "{$url}"; ?>" <?php if ($row['window'] !== '') { ?><?php echo "{$row['window']}"; ?><?php }; ?> class="nav-section-link <?php if ($row['rednote'] !== ''){ ?>nav-section-subtext-anchor<?php }; ?> <?php if ($row['window'] == '') { ?>iframe-link<?php }; ?>" onclick="javascript:pageTracker._trackPageview('ColumnLink/<?php echo "{$row['title']}"; ?>'); "><?php echo "{$row['title']}"; ?></a><?php if ($row['rednote'] !== '') { ?><div class="nav-section-subtext"><p>&nbsp;&nbsp;<?php echo "{$row['rednote']}"; ?></p></div><?php }; ?></td>
	      <td></td>
	    </tr>
<?php
	}
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT `id` FROM documents WHERE doctype = '$doctype' AND owner = 'Y' AND type LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY title LIMIT 1";
	$result = mysqli_query($dbConn, $query);
...
<a class="image-popup-no-margins" href="../download-documents.php?id=<?php echo "{$row['id']}"; ?>&docdate=<?php echo "{$row['docdate']}"; ?>&size=<?php echo "{$row['size']}"; ?>&conn=<?php echo "{$connName}";?>" alt="<?php echo "{$row['title']}"; ?>&conn=<?php echo "{$connName}";?>" title="<?php echo "{$row['title']}"; ?>" target="_blank" onclick="javascript:pageTracker._trackPageview('DOCUMENTS/<?php echo "{$row['id']}"; ?>'); ">
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT `id` FROM documents WHERE doctype = '$doctype' AND lease = 'Y' AND type LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY title LIMIT 1";
	$result = mysqli_query($dbConn, $query);
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT `id` FROM documents WHERE doctype = '$doctype' AND realtor = 'Y' AND type LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY title LIMIT 1";
	$result = mysqli_query($dbConn, $query);
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT `id` FROM documents WHERE doctype = '$doctype' AND public = 'Y' AND type LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY title LIMIT 1";
	$result = mysqli_query($dbConn, $query);
