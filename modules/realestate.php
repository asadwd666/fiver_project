...
<div id="all-documents-folder" class="stand-alone-page">
  <div class="popup-header">
    <h4>
<?php
	$query  = "SELECT title FROM tabs WHERE `int1` = '208'";
...
<?php
	...
	$query  = "SELECT * FROM realestate WHERE (forsalerent = 'SALE' OR forsalerent = 'RENT') AND approved = 'Y' AND eod >= CURRENT_DATE() ORDER BY created_date DESC";
	$result = mysqli_query($dbConn, $query);
...
<span class="text"><?php if ($row['url'] !== ''){ ?><a href="<?php echo "{$row['url']}"; ?>" target="_blank" onclick="javascript:pageTracker._trackPageview('External/<?php echo "{$row['url']}"; ?>'); ">Link to listing/ad</a><?php }; ?>
<?php if ($row['docid'] !== ''){ ?>
	<?php
		$type    = $row['docid'];
