...
<div id="all-documents-folder" class="stand-alone-page">
  <div class="popup-header">
    <h4>
<?php
	$query  = "SELECT title FROM tabs WHERE `int1` = '204'";
...
<!-- Mobile Filter Notice -->
  <div class="stand-alone-page-content">
    <div class="rotate-note">Rotate your device to filter listings by category.</div>
  </div>
...
<!-- POST CONTENT NOTICE -->
    ...
    <tr>
      ...
      <th align="left" width="125" class="table-sortable:alphanumeric table-filterable" style="cursor:pointer;"><small>Type</small></th>
    </tr>
  </thead>
...
<!-- DATABASE RESULTS -->
<?php
	$query  = "SELECT * FROM realestate WHERE eod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() AND approved = 'Y' ORDER BY created_date DESC";
	$result = mysqli_query($dbConn, $query);
...
<?php if ($row['email'] !== ''){ ?><br>Email: <a href="mailto:<?php echo "{$row['email']}"; ?>" onclick="javascript:pageTracker._trackPageview('RealEstate/<?php echo "{$row['email']}"; ?>'); "><?php echo "{$row['email']}"; ?></a><?php }; ?></span>
	      </td>
	      <td align="left" valign="top"><?php if ($row['forsalerent'] == 'CLASSIFIED'){ ?>Item for Sale<?php }; ?><?php if ($row['forsalerent'] == 'SALE'){ ?>Unit for Sale<?php }; ?><?php if ($row['forsalerent'] == 'RENT'){ ?>Unit for Rent<?php }; ?></td>
	    </tr>
<?php
	}
?>
