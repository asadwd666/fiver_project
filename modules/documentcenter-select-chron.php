...
<?php
	...
	$query  = "SELECT `id` FROM documents INNER JOIN tabs INNER JOIN folders WHERE (documents.doctype = '$choice' AND documents.owner = 'Y' AND type NOT LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE()) OR (tabs.tabname = '$choice' AND tabs.owner = 'Y') OR (folders.tabname = '$choice' AND folders.owner = 'Y') LIMIT 1";
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT `id`, `docdate`, `size`, `title`, `type` FROM documents WHERE doctype = '$doctype' AND owner = 'Y' AND `type` NOT LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY docdate DESC";
	$result = mysqli_query($dbConn, $query);
...
?>
	    <tr>
	      <td align="left"><?php include('../icon-links.php'); ?>&nbsp;&nbsp;&nbsp;
              <a href="../download-documents.php?id=<?php echo "{$row['id']}"; ?>&docdate=<?php echo "{$row['docdate']}"; ?>&size=<?php echo "{$row['size']}"; ?><?php echo "&conn={$connName}";?>" target="_blank" onclick="javascript:pageTracker._trackPageview('DOCUMENTS/<?php echo "{$row['id']}"; ?>'); "><?php echo "{$row['title']}"; ?></a></span></td>
...
?>
	    <tr>
	      <td align="left"><a href="<?php echo "{$link}"; ?><?php echo "{$row['title']}"; ?><?php echo "&conn={$connName}";?>" class="iframe-link" onclick="javascript:pageTracker._trackPageview('FOLDER/<?php echo "{$row['title']}"; ?>'); "><?php echo "{$row['image']}"; ?></a>&nbsp;&nbsp;
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT `id` FROM documents WHERE doctype = '$doctype' AND owner = 'Y' AND `type` LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY docdate DESC LIMIT 1";
	$result = mysqli_query($dbConn, $query);
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT * FROM documents WHERE doctype = '$doctype' AND owner = 'Y' AND `type` LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY docdate DESC";
	$result = mysqli_query($dbConn, $query);
...
<div class="small-2 large-2 columns">
  <div align="right" class="gallery-download"><p>
          <a href="../download-documents.php?id=<?php echo "{$row['id']}"; ?>&docdate=<?php echo "{$row['docdate']}"; ?>&size=<?php echo "{$row['size']}"; ?>&conn=<?php echo "{$connName}";?>" target="_blank" onclick="javascript:pageTracker._trackPageview('DOCUMENTS/<?php echo "{$row['id']}"; ?>'); "><i class="fa fa-download" aria-hidden="true" title="Download"></i></a></p></div>
</div>
...
<?php if ($_SESSION["liaison"] == true) { ?>
    <tr style="background-color: #FAFEB8;">
      <th colspan="2"><p>Preview of this folder with lease permissions.</p></th>
    </tr>
<?php } ?>
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT `id`, `docdate`, `size`, `title`, `type` FROM documents WHERE doctype = '$doctype' AND lease = 'Y' AND `type` NOT LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY docdate DESC";
	$result = mysqli_query($dbConn, $query);
...
?>
	    <tr>
	      <td align="left"><?php include('../icon-links.php'); ?>
              <a href="../download-documents.php?id=<?php echo "{$row['id']}"; ?>&docdate=<?php echo "{$row['docdate']}"; ?>&size=<?php echo "{$row['size']}"; ?>&conn=<?php echo"{$connName}";?>" target="_blank" onclick="javascript:pageTracker._trackPageview('DOCUMENTS/<?php echo "{$row['id']}"; ?>'); "><?php echo "{$row['title']}"; ?></a></span></td>
	      <td align="left"><?php echo date('Y', strtotime($row['docdate'])); ?></td>
	    </tr>
<?php
	}
...
?>
	    <tr>
	      <td align="left">
              <a href="<?php echo "{$link}"; ?><?php echo "{$row['title']}"; ?>&conn="<?php echo "{$connName}";?>" class="iframe-link" onclick="javascript:pageTracker._trackPageview('FOLDER/<?php echo "{$row['title']}"; ?>'); "><?php echo "{$row['image']}"; ?></a>&nbsp;
              <a href="<?php echo "{$link}"; ?><?php echo "{$row['title']}"; ?>&conn="<?php echo "{$connName}";?>" class="nav-section-link <?php if ($row['rednote'] !== '') { ?>nav-section-subtext-anchor<?php }; ?> iframe-link" onclick="javascript:pageTracker._trackPageview('FOLDER/<?php echo "{$row['title']}"; ?>'); "><b><?php echo "{$row['title']}"; ?></b></a><?php if ($row['rednote'] !== '') { ?><div class="nav-section-subtext"><p>&nbsp;&nbsp;<?php echo "{$row['rednote']}"; ?></p></div><?php }; ?></td>
	      <td></td>
	    </tr>
<?php
	}
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT `id` FROM documents WHERE doctype = '$doctype' AND lease = 'Y' AND `type` LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY docdate DESC LIMIT 1";
	$result = mysqli_query($dbConn, $query);
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT * FROM documents WHERE doctype = '$doctype' AND lease = 'Y' AND `type` LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY docdate DESC";
	$result = mysqli_query($dbConn, $query);
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT `id`, `docdate`, `size`, `title`, `type` FROM documents WHERE doctype = '$doctype' AND realtor = 'Y' AND `type` NOT LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY docdate DESC";
	$result = mysqli_query($dbConn, $query);
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT `id` FROM documents WHERE doctype = '$doctype' AND realtor = 'Y' AND `type` LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY docdate DESC LIMIT 1";
	$result = mysqli_query($dbConn, $query);
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT * FROM documents WHERE doctype = '$doctype' AND realtor = 'Y' AND `type` LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY docdate DESC";
	$result = mysqli_query($dbConn, $query);
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT `id`, `docdate`, `size`, `title`, `type` FROM documents WHERE doctype = '$doctype' AND public = 'Y' AND type NOT LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY docdate";
	$result = mysqli_query($dbConn, $query);
...
<?php
	$tabname    = $_GET['choice'];
	$query  = "SELECT * FROM tabs WHERE tabname = '$doctype' AND public = 'Y' ORDER BY title";
	$result = mysqli_query($dbConn,$query) or die('Error, query failed');
...
<?php
	$foldername    = $_GET['choice'];
	$query  = "SELECT * FROM folders WHERE tabname = '$foldername' AND public = 'Y' ORDER BY title";
	$result = mysqli_query($dbConn,$query) or die('Error, query failed');
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT `id` FROM documents WHERE doctype = '$doctype' AND public = 'Y' AND `type` LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY docdate DESC LIMIT 1";
	$result = mysqli_query($dbConn, $query);
...
<?php
	$doctype    = $_GET['choice'];
	$query  = "SELECT * FROM documents WHERE doctype = '$doctype' AND public = 'Y' AND type LIKE '%image%' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY docdate";
	$result = mysqli_query($dbConn, $query);
...
<div class="small-3 large-3 columns">
  <div align="right" class="gallery-download"><p><a href="../download-documents.php?id=<?php echo "{$row['id']}"; ?>&docdate=<?php echo "{$row['docdate']}"; ?>&size=<?php echo "{$row['size']}"; ?>" target="_blank" onclick="javascript:pageTracker._trackPageview('DOCUMENTS/<?php echo "{$row['id']}"; ?>'); "><i class="fa fa-download" aria-hidden="true"></i> Download</a></p></div>
</div>
