...
<?php
	...
	{
?>
<?php if ($_GET["choice"] == 'Utility') { ?>
<?php
	$query  = "SELECT title FROM tabs WHERE `int1` = '311'";
...
<?php if ($_GET["choice"] == 'Local Links') { ?>
<?php
	$query  = "SELECT title FROM tabs WHERE `int1` = '312'";
	$result = mysqli_query($dbConn, $query);
...
<?php
	...
	$query  = "SELECT * FROM utilities WHERE category = '$category' ORDER BY company";
	$result = mysqli_query($dbConn, $query);
...
<div class="row">
    <div class="small-12 columns">
	    <div style="margin-top: 85px; margin-bottom: 1px;">
	        <?php if ($row['name'] !== '' AND $row['name'] !== 'none.gif'){ ?><img src="../download-utilities.php?id=<?php echo "{$row['id']}"; ?>" alt="<?php echo "{$row['company']}"; ?>"><?php }; ?>
	    </div>
    </div>
