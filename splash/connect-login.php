...
/* Query database for email address */
        ...
        $countEMAIL = $row[0];
        if ($countEMAIL == '0'){ $error = '<br><i class="fa fa-exclamation-triangle note" aria-hidden="true" title="Action!"></i> Invalid email address or password.<br>'; };
...
/* Verify hashed password with typed password */
		...
		if (($_POST['email'] = $rowHASH['email']) AND ($_POST['password']= $rowHASH['password'])) {
...
/* Check account status */
				if($row['status'] == "new") {
					...
					$error = '<br><i class="fa fa-exclamation-triangle note" aria-hidden="true" title="Action!"></i> Your account is disabled.<br><a href="connect-disabled.php?choice=disabled" class="iframe-link"><u>Request Reactivation</u></a><br><br>';
				} else if($row['accessdate'] >= '0000-00-01' && $row['accessdate'] <= $currentdate) {
					$error = '<br><i class="fa fa-exclamation-triangle note" aria-hidden="true" title="Action!"></i> You have reached the end of your lease/access through date, and your account was suspended.<br><a href="connect-disabled.php?choice=suspended" class="iframe-link"><u>Request Reactivation</u></a><br><br>';
...
/* Error for Email Address and Password */
	...
	$error = '<br><i class="fa fa-exclamation-triangle note" aria-hidden="true" title="Action!"></i> Invalid email address or password.<br><span style="text-decoration: underline;"><a href="connect-help.php" class="iframe-link">Reset your password or get login help here.</a></span><br>';
	}
	}
}
?>
	...
	<form method="POST">
        ...
        <div class="row">
          ...
          <div class="small-9 medium-9 large-9 end columns"><input name="password" class="form" type="password" id="pass" maxlength="100">
              <div style="margin-top: -39px; margin-right: 12px;" required><i class="fa fa-eye" id="togglePassword" style="color: #000; cursor: pointer; float: right;position: relative;z-index: 1"></i> </div>
          </div>
        ...
        <div class="row">
          <div class="small-12 columns text-center">
            <br><small><a href="connect-request.php" class="iframe-link">Create a login</a> | <a href="connect-help.php" class="iframe-link">Login Help?</a></small>
	  </div>
        </div>
...
Is this your first visit? <b><a href="connect-request.php" class="iframe-link">Create a login</a></b>.<br>
<br>
Need help? <b><a href="connect-help.php" class="iframe-link"> Forgot your login email address or password?</a></b>
