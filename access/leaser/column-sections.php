<?php
    ...
    $queryNAVIGATION  = "SELECT type FROM navigation where lease = 'Y' and type != 'Documents'";
...
<?php
	$navigation    = $rowNAVIGATION['type'];
	$query  = "SELECT `id`, type, title, aod, docdate, size FROM documents WHERE doctype = '$navigation' AND lease = 'Y' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY title";
        $dataArray = array();
        foreach ($connectionPool as $connName => $configuration) {
...
<?php
	$navigation    = $rowNAVIGATION['type'];
        $query  = "SELECT `int1`, tabname, title, rednote, image, url, window FROM tabs WHERE tabname = '$navigation' AND lease = 'Y' ORDER BY title";
        $dataArray = array();
        foreach ($connectionPool as $connName => $configuration) {
            $resultARRAY = mysqli_query($configuration['connection'],$query);
...
<?php
	$navigation    = $rowNAVIGATION['type'];
        $query  = "SELECT * FROM folders WHERE tabname = '$navigation' AND lease = 'Y' ORDER BY title";
        $dataArray = array();
        foreach ($connectionPool as $connName => $configuration) {
            $resultARRAY = mysqli_query($configuration['connection'],$query);
