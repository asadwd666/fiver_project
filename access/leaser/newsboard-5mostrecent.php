...
foreach ($localConnectionPool as $configuration) {
    ...
    $countDOCCOUNT = 0;
    $query = "SELECT count(*) FROM `documents` WHERE lease = 'Y' AND (type != 'image/jpeg' AND type != 'image/pjpeg' AND type != 'image/gif' AND type != 'image/png') AND created_date BETWEEN NOW() - INTERVAL 60 DAY AND NOW() AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE()";
...
<!-- Content -->
  	<?php
        $dataArray = array();
        $query  = "SELECT `id`, type, title, aod, docdate, doctype, created_date, docdate, size FROM `documents` WHERE lease = 'Y' AND (type != 'image/jpeg' AND type != 'image/pjpeg' AND type != 'image/gif' AND type != 'image/png') AND created_date BETWEEN NOW() - INTERVAL 60 DAY AND NOW() AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY created_date DESC LIMIT 5";
        $result = mysqli_query($configuration['connection'], $query);
