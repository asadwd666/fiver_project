<?php
    ...
    $dataArray = array();
	$queryNEWS  = "SELECT *, UNIX_TIMESTAMP(CONCAT(pod)) as result_index  FROM chalkboard WHERE (lease = 'Y' AND eod >= CURRENT_DATE() AND pod <= CURRENT_DATE()) AND (`flag` = 'U' OR `flag` = 'M' OR `flag` = 'A' OR `flag` = 'I' OR `flag` = 'D' OR `flag` = 'C' OR `flag` = 'R') ORDER BY pod DESC";
