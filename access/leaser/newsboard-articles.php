<?php
    ...
    $dataArray = array();
    $query  = "SELECT *, UNIX_TIMESTAMP(CONCAT(pod)) as result_index FROM chalkboard WHERE (lease = 'Y' AND eod >= CURRENT_DATE() AND pod <= CURRENT_DATE()) AND (`flag` != 'U' AND `flag` != 'M' AND `flag` != 'A' AND `flag` != 'I' AND `flag` != 'D' AND `flag` != 'C' AND `flag` != 'R') ORDER BY pod DESC";
