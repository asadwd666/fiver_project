...
<?php
    ...
    $query  = "SELECT `id`, type, title, aod, docdate, size FROM documents WHERE doctype = 'PropertyManager' AND lease = 'Y' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY title";
    foreach ($connectionPool as $connName => $configuration) {
        $resultARRAY = mysqli_query($configuration['connection'],$query);
...
<?php
    ...
    $dataArray = array();
   // $navigation    = $rowNAVIGATION['type'];
    $query  = "SELECT `int1`, tabname, title, rednote, image, url, window FROM tabs WHERE tabname = 'PropertyManager' AND lease = 'Y' ORDER BY title";
    foreach ($connectionPool as $connName => $configuration) {
        $resultARRAY = mysqli_query($configuration['connection'],$query);
...
<?php
    ...
    $dataArray = array();
    //$navigation    = $rowNAVIGATION['type'];
    $query  = "SELECT * FROM folders WHERE tabname = 'PropertyManager' AND lease = 'Y' ORDER BY title";
    foreach ($connectionPool as $connName => $configuration) {
        $resultARRAY = mysqli_query($configuration['connection'],$query);
