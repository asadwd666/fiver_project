...
?>
<!-- Section -->
        <div class="nav-section-header nav-section-header__red">
          <div class="row">
            <div class="small-12 columns"><h3 class="nav-section-title">Administrators</h3></div>
          </div>
        </div>
...
<?php
	...
	$query  = "SELECT `int1`, tabname, title, rednote, image, url, window FROM tabs WHERE tabname = 'Liaison' AND owner = 'Y' AND `int1` = '1001' ORDER BY title";
...
<?php
	$navigation    = $rowNAVIGATION['type'];
	$query  = "SELECT `id`, type, title, aod, docdate, size FROM documents WHERE doctype = 'Liaison' AND owner = 'Y' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY title";
...
?>
		<ul class="nav-section-links">
		<li><?php include('../icon-links.php'); ?><a href="../download-documents.php?id=<?php echo "{$row['id']}"; ?>&docdate=<?php echo "{$row['docdate']}"; ?>&size=<?php echo "{$row['size']}"; ?>&conn=<?php echo "{$connName}";?>" class="nav-section-link" target="_blank" onclick="javascript:pageTracker._trackPageview('DOCUMENTS/<?php echo "{$row['id']}"; ?>'); "><?php echo "{$row['title']}"; ?></a></li>
		</ul>
<?php
	        }
        }
...
<?php
	$navigation    = $rowNAVIGATION['type'];
	$query  = "SELECT * FROM folders WHERE tabname = 'Liaison' AND owner = 'Y' ORDER BY title";
