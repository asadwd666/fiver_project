<?php
    ...
    $dataArray = array();
	$queryNEWS  = "SELECT *, UNIX_TIMESTAMP(CONCAT(pod)) as result_index FROM chalkboard WHERE (owner = 'Y' AND eod >= CURRENT_DATE() AND pod <= CURRENT_DATE()) AND (`flag` = 'U' OR `flag` = 'M' OR `flag` = 'A' OR `flag` = 'I' OR `flag` = 'D' OR `flag` = 'C' OR `flag` = 'R') ORDER BY pod DESC";
	//$resultNEWS = mysqli_query($conn,$queryNEWS);


    foreach ($connectionPool as $connName => $configuration) {
        $resultARRAY = mysqli_query($configuration['connection'],$queryNEWS);
    ...
    krsort($dataArray, 1);
    foreach($dataArray as $key=>$recordArray ) {
        ksort($recordArray, 1);
...
<?php if ($row['pic'] != '' AND ($row['pic2'] == '' AND $row['pic3'] == '')){ ?>
	<?php
		...
		while($rowImg = $result->fetch_array(MYSQLI_ASSOC))
		{
	?>
		<div class="newsboard-post-image-solo">
            <a href="../download-documents-mini.php?id=<?php echo "{$rowImg['id']}"; ?>&docdate=<?php echo "{$rowImg['docdate']}"; ?>&size=<?php echo "{$rowImg['size']}"; ?>&conn=<?php echo $record['dbconnName']; ?>" class="iframe-link" >
                <img src="../download-documents-mini.php?id=<?php echo "{$rowImg['id']}"; ?>&docdate=<?php echo "{$rowImg['docdate']}"; ?>&size=<?php echo "{$rowImg['size']}"; ?>&conn=<?php echo $record['dbconnName']; ?>" alt="<?php echo "{$rowImg['title']}"; ?>" title="<?php echo "{$rowImg['title']}"; ?>">
            </a>
        </div>
	<?php
		}
...
<!-- URL -->
	<?php if ($row['url'] != ''){ ?>
	<li>
		<a href="<?php echo "{$row['url']}"; ?>" target="_blank" onclick="javascript:pageTracker._trackPageview('Chalkboard/<?php echo "{$row['url']}"; ?>'); "><i class="fa fa-link" aria-hidden="true"></i></a>
...
<!-- MODULE -->
	...
	?>
	<li>
		<a href="<?php echo "{$rowTID['url']}"; ?>" <?php if ($rowTID['window'] !== '') { ?><?php echo "{$rowTID['window']}"; ?><?php }; ?> class="<?php if ($rowTID['window'] == '') { ?>iframe-link<?php }; ?>" ><?php echo "{$rowTID['image']}"; ?></a>
...
<!-- FOLDER -->
	...
	?>
	<li>
		<a href="<?php echo "{$rowFID['link']}"; ?><?php echo "{$rowFID['title']}"; ?>" class="iframe-link" onclick="javascript:pageTracker._trackPageview('FOLDER/<?php echo "{$rowFID['title']}"; ?>'); "><?php echo "{$rowFID['image']}"; ?></a>
...
<!-- CALENDAR -->
	...
	?>
	<li>
		<a href="../modules/events-single.php?choice=<?php echo "{$rowCID['int1']}"; ?>" class="iframe-link"><i class="fa fa-calendar-o" aria-hidden="true"></i></a>
	<a href="../modules/events-single.php?choice=<?php echo "{$rowCID['int1']}"; ?>" class="iframe-link"><?php echo "{$rowCID['title']}"; ?> <?php echo "{$rowCID['date']}"; ?></a></li>
	<?php
		}
	?>
	<?php }; ?>
...
<!-- DOCUMENT 1 -->
	...
	?>
	<li>
        <a href="../download-documents-embed.php?table=chalkboard&mid=<?php echo "{$row['int1']}"; ?>&date=<?php echo date('Y-m-d', strtotime($row['created_date'])); ?>&d1=Y&conn=<?php echo $record['dbconnName']; ?>" target="_blank" onclick="javascript:pageTracker._trackPageview('DOCUMENTS/<?php echo "{$row1['id']}"; ?>')"><?php include('../icon-links-embed.php'); ?></a>
...
<!-- DOCUMENT 2 -->
	...
	?>
	<li>
        <a href="../download-documents-embed.php?table=chalkboard&mid=<?php echo "{$row['int1']}"; ?>&date=<?php echo date('Y-m-d', strtotime($row['created_date'])); ?>&d2=Y&conn=<?php echo $record['dbconnName']; ?>" target="_blank" onclick="javascript:pageTracker._trackPageview('DOCUMENTS/<?php echo "{$row1['id']}"; ?>')"><?php include('../icon-links-embed.php'); ?></a>
...
<!-- DOCUMENT 3 -->
	...
	?>
            <li>
            	<a href="../download-documents-embed.php?table=chalkboard&mid=<?php echo "{$row['int1']}"; ?>&date=<?php echo date('Y-m-d', strtotime($row['created_date'])); ?>&d3=Y&conn=<?php echo $record['dbconnName']; ?>" target="_blank" onclick="javascript:pageTracker._trackPageview('DOCUMENTS/<?php echo "{$row1['id']}"; ?>')"><?php require('../icon-links-embed.php'); ?></a>
              <a href="../download-documents-embed.php?table=chalkboard&mid=<?php echo "{$row['int1']}"; ?>&date=<?php echo date('Y-m-d', strtotime($row['created_date'])); ?>&d3=Y&conn=<?php echo $record['dbconnName']; ?>" target="_blank" onclick="javascript:pageTracker._trackPageview('DOCUMENTS/<?php echo "{$row1['id']}"; ?>')">Download&nbsp;<?php echo "{$row1['title']}"; ?></a>
            </li>
	<?php
		}
	?>
...
<?php if ($row['pic'] == '' AND ($row['pic2'] != '' AND $row['pic3'] != '')){ ?>
	...
	<?php
		...
		while($rowPic2 = $result->fetch_array(MYSQLI_ASSOC))
		{
	?>
            <div class="newsboard-post-image-twins-1">
                <a href="../download-documents-mini.php?id=<?php echo "{$rowPic2['id']}"; ?>&docdate=<?php echo "{$rowPic2['docdate']}"; ?>&size=<?php echo "{$rowPic2['size']}"; ?>&conn=<?php echo $record['dbconnName']; ?>" class="iframe-link" >
                    <img src="../download-documents-mini.php?id=<?php echo "{$rowPic2['id']}"; ?>&docdate=<?php echo "{$rowPic2['docdate']}"; ?>&size=<?php echo "{$rowPic2['size']}"; ?>&conn=<?php echo $record['dbconnName']; ?>" alt="<?php echo "{$rowPic2['title']}"; ?>" title="<?php echo "{$rowPic2['title']}"; ?>"></a></div>
	<?php
		}
	?>
      </div>
	...
	<?php
		...
		while($rowPic3 = $result->fetch_array(MYSQLI_ASSOC))
		{
	?>
            <div class="newsboard-post-image-twins-2">
                <a href="../download-documents-mini.php?id=<?php echo "{$rowPic3['id']}"; ?>&docdate=<?php echo "{$rowPic3['docdate']}"; ?>&size=<?php echo "{$rowPic3['size']}"; ?>&conn=<?php echo $record['dbconnName']; ?>" class="iframe-link" >
                    <img src="../download-documents-mini.php?id=<?php echo "{$rowPic3['id']}"; ?>&docdate=<?php echo "{$rowPic3['docdate']}"; ?>&size=<?php echo "{$rowPic3['size']}"; ?>&conn=<?php echo $record['dbconnName']; ?>" alt="<?php echo "{$rowPic3['title']}"; ?>" title="<?php echo "{$rowPic3['title']}"; ?>"></a></div>
	<?php
		}
	?>
      </div>
