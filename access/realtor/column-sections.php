<?php
    ...
    $queryNAVIGATION  = "SELECT type FROM navigation where realtor = 'Y' and type != 'Documents'";
...
<?php
    ...
    $query  = "SELECT `id` FROM documents INNER JOIN tabs INNER JOIN folders WHERE (documents.aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() AND documents.doctype = '$navigation' AND documents.realtor = 'Y') OR (tabs.tabname = '$navigation' AND tabs.realtor = 'Y') OR (folders.tabname = '$navigation' AND folders.realtor = 'Y') LIMIT 1";
...
<?php
	$navigation    = $rowNAVIGATION['type'];
        $query  = "SELECT `id`, type, title, aod, docdate, size FROM documents WHERE doctype = '$navigation' AND realtor = 'Y' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY title";
        $dataArray = array();
        foreach ($connectionPool as $connName => $configuration) {
...
<?php
	$navigation    = $rowNAVIGATION['type'];
        $query  = "SELECT `int1`, tabname, title, rednote, image, url, window FROM tabs WHERE tabname = '$navigation' AND realtor = 'Y' ORDER BY title";
        $dataArray = array();
        foreach ($connectionPool as $connName => $configuration) {
            $resultARRAY = mysqli_query($configuration['connection'],$query);
...
<?php
	$navigation    = $rowNAVIGATION['type'];
        $query  = "SELECT * FROM folders WHERE tabname = '$navigation' AND realtor = 'Y' ORDER BY title";
        $dataArray = array();
        foreach ($connectionPool as $connName => $configuration) {
            $resultARRAY = mysqli_query($configuration['connection'],$query);
