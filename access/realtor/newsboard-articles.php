<?php
    ...
    $dataArray = array();
    $query  = "SELECT *, UNIX_TIMESTAMP(CONCAT(pod)) as result_index FROM chalkboard WHERE (realtor = 'Y' AND eod >= CURRENT_DATE() AND pod <= CURRENT_DATE()) AND (`flag` != 'U' AND `flag` != 'M' AND `flag` != 'A' AND `flag` != 'I' AND `flag` != 'D' AND `flag` != 'C' AND `flag` != 'R') ORDER BY pod DESC";
...
<!-- CALENDAR -->
	...
	<?php
		...
		{
	?>
	<li><a href="../modules/events-single.php?choice=<?php echo "{$rowCID['int1']}"; ?>&conn=<?php echo $record['dbconnName']; ?>" class="iframe-link"><i class="fa fa-calendar-o" aria-hidden="true"></i></a>
	<a href="../modules/events-single.php?choice=<?php echo "{$rowCID['int1']}";?>&conn=<?php echo $record['dbconnName']; ?>" class="iframe-link"><?php echo "{$rowCID['title']}"; ?> <?php echo "{$rowCID['date']}"; ?></a></li>
	<?php
		}
	?>
	<?php }; ?>
