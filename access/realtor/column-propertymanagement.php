...
<?php
    ...
    $query  = "SELECT `id`, type, title, aod, docdate, size FROM documents WHERE doctype = 'PropertyManager' AND realtor = 'Y' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY title";
    foreach ($connectionPool as $connName => $configuration) {
        $resultARRAY = mysqli_query($configuration['connection'],$query);
...
?>
		...
		<li style="margin-left: -14px;"><?php include('../icon-links.php'); ?><a href="../download-documents.php?id=<?php echo "{$row['id']}"; ?>&docdate=<?php echo "{$row['docdate']}"; ?>&size=<?php echo "{$row['size']}"; ?>" target="_blank" class="nav-section-link" onclick="javascript:pageTracker._trackPageview('DOCUMENTS/<?php echo "{$row['id']}"; ?>'); "><?php echo "{$row['title']}"; ?></a></li>
		</ul>
<?php
	        }
        }
...
<?php
    ...
    $dataArray = array();
    $navigation    = $rowNAVIGATION['type'];
    $query  = "SELECT `int1`, tabname, title, rednote, image, url, window FROM tabs WHERE tabname = 'PropertyManager' AND realtor = 'Y' ORDER BY title";
    foreach ($connectionPool as $connName => $configuration) {
        $resultARRAY = mysqli_query($configuration['connection'],$query);
...
<?php
    ...
    $dataArray = array();
    $navigation    = $rowNAVIGATION['type'];
    $query  = "SELECT * FROM folders WHERE tabname = 'PropertyManager' AND realtor = 'Y' ORDER BY title";
    foreach ($connectionPool as $connName => $configuration) {
        $resultARRAY = mysqli_query($configuration['connection'],$query);
