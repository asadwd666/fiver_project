<?php
	...
	$query  = "SELECT `id` FROM documents INNER JOIN tabs INNER JOIN folders WHERE (documents.aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() AND documents.doctype = 'Meeting Minutes' AND documents.realtor = 'Y') OR (tabs.tabname = 'Meeting Minutes' AND tabs.realtor = 'Y') OR (folders.tabname = 'Meeting Minutes' AND folders.realtor = 'Y') LIMIT 1";
    $dataArray = array();
...
<!-- Section -->
        <div class="nav-section-header">
          <div class="row">
            <div class="small-12 columns"><h3 class="nav-section-title">2 Most Recent Meeting Minutes</h3></div>
          </div>
        </div>
<?php
	        }
...
<?php
	...
	$query  = "SELECT UNIX_TIMESTAMP(docdate) as sort_index, `id`, type, title, aod, docdate, size FROM documents WHERE doctype = 'Meeting Minutes' AND realtor = 'Y' AND aod NOT BETWEEN '0000-01-01' AND CURRENT_DATE() ORDER BY `docdate` DESC LIMIT 2";
...
<?php
	$navigation    = $rowNAVIGATION['type'];
	$query  = "SELECT `int1`, tabname, title, rednote, image, url, window FROM tabs WHERE tabname = 'Meeting Minutes' AND realtor = 'Y' ORDER BY title";
...
<?php
	$navigation    = $rowNAVIGATION['type'];
	$query  = "SELECT * FROM folders WHERE tabname = 'Meeting Minutes' AND realtor = 'Y' ORDER BY title";
