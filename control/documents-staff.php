<?php $current_page = '13'; include('protect.php'); ?>
<!DOCTYPE html>
...
<?php
	if ($action == "delete"){
		...
		$id = $_POST['id'];
		$comment = $_POST['title'];
		$query = "INSERT INTO log (`action`, `tablename`, `useripaddress`, `userid`, `id`, `comment`) VALUES ('D', 'Documents-Staff', '$useripaddress', '$userid', '$id', '$comment')";
	...
	if ($action == "recover"){
	    $location = "documents-staff.php".$folder;
		$id = $_POST["id"];
		$aod = date('Y-m-d');
		...
		$id = $_POST['id'];
		$comment = $_POST['title'];
		$query = "INSERT INTO log (`action`, `tablename`, `useripaddress`, `userid`, `id`, `comment`) VALUES ('R', 'Documents-Staff', '$useripaddress', '$userid', '$id', '$comment')";
		mysqli_query($conn,$query) or die('Error, updating log failed');
	...
	if ($action == "upload" && $_FILES['userfile']['size'] > 0){
		...
		$query  = "INSERT INTO documents (name, size, type, content, docdate, doctype, title, board, aod, useripaddress, userid) VALUES ('$fileName', '$fileSize', '$fileType', '$content', '$docdate', '$doctype', '$title', '$board', '$aod', '$useripaddress', '$userid')";
...
<big><i class="fa fa-stethoscope" aria-hidden="true"></i></big><strong>&nbsp;Health&nbsp;&nbsp;&nbsp;</strong> Documents for Concierge/Staff do not impact Health rating.
        </div>
    </div>
...
<div class="row cp-help">
    <div class="small-12 medium-6 columns">
        <p>This is where you upload <b>documents and photos</b> for <u><b>staff use only</b></u>.</p>
        <p>
...
<form enctype="multipart/form-data" method="POST" action="documents-staff.php">
...
1st Amendment to Declaration<br>
    ...
    <div class="small-12 medium-12 large-6 columns">
        <div class="row" style="padding: 10px 10px 0px 0px;">
            <div class="small-12 medium-12 columns"><strong>3) Dates!</strong></div>
        </div>
        <div class="row medium-collapse" style="padding-left: 30px;">
        ...
        <div class="row" style="padding: 10px 10px 10px 0px;">
            <div class="small-12 medium-12 columns"><strong>4) Who should have access to this document?</strong></div>
        </div>
        <div class="row medium-collapse" style="padding-left: 30px;">
            <div class="small-12 medium-6 columns"><label for="board" class="middle">Show in <b>Board</b> Folder?</label></div>
...
<option value="Y">Yes</option>
        ...
        <div class="row" style="padding: 10px 10px 10px 0px;">
            ...
            <div class="small-12 medium-6 columns">
<input type="hidden" name="doctype" value="Staff">
...
<br>
<div class="nav-section-header-cp">
        <strong><?php $sql = mysqli_query($conn,"SELECT count(*) FROM documents WHERE doctype = 'Staff' AND (aod = '0000-00-00' OR aod >= CURRENT_DATE())") or die(mysqli_error($conn));
...
<?php
	$today = date('Y-m-d');
	$query  = "SELECT `id`, type, title, docdate, doctype, owner, lease, realtor, public, board, size, created_date, userid, aod FROM documents WHERE doctype = 'Staff' AND (aod = '0000-00-00' OR aod >= CURRENT_DATE()) ORDER BY `id` DESC";
...
?>
        ...
        <div class="small-6 medium-6 large-2 columns">
            <form name="DocumentsEdit" method="POST" action="documents-edit-staff.php">
                <input type="hidden" name="action" value="edit">
                <input type="hidden" name="id" value="<?php echo "{$row['id']}"; ?>">
                <?php if ($foldername !== ''){ ?><input type="hidden" name="module" value="documents-staff.php?folder=<?php echo "{$row['doctype']}"; ?>#edit"><?php }; ?>
                <?php if ($foldername == ''){ ?><input type="hidden" name="module" value="documents-staff.php#edit"><?php }; ?>
        ...
        <?php if ($row['aod'] !== $today){ ?>
        <div class="small-6 medium-6 large-2 columns">
            <form method="POST" action="documents-staff.php" onclick="return confirm('Are you sure you want to delete the document: <?php echo "{$row['title']}"; ?>?');">
        ...
        <div class="small-12 medium-12 large-4 columns" align="center">
            <form method="POST" action="documents-staff.php" onclick="return confirm('Are you sure you want to recover the document: <?php echo "{$row['title']}"; ?>?');">
...
<div class="small-12 medium-12 large-12 columns note-black"><br><br>Documents (Staff) Control Panel Page<br></div>
</body>
</html>
