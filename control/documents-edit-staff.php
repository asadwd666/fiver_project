<?php $current_page = '13'; include('protect.php'); $id = $_POST["id"]; $action = $_POST["action"];
	if ($action == "save"){
		...
		$query = "UPDATE documents SET type='$type', doctype='$doctype', title='$title', docdate='$docdate', board='$board', aod='$aod' WHERE `id`='$id' LIMIT 1";
		mysqli_query($conn,$query) or die('Error, update query failed');

        $useripaddress = $_SERVER['REMOTE_ADDR'];
		$userid = $_SESSION['id'];
		$id = $_POST['id'];
		$query = "INSERT INTO log (action, tablename, useripaddress, userid, id) VALUES ('E', 'Documents-Staff', '$useripaddress', '$userid', '$id')";
		mysqli_query($conn,$query) or die('Error, updating log failed');

		header('Location: documents-staff.php');
	}
...
<?php
	$query  = "SELECT `id`, name, size, type, created_date, doctype, title, owner, lease, realtor, public, board, docdate, aod FROM documents WHERE `id`='$id' LIMIT 1";
...
<form enctype="multipart/form-data" method="POST" action="documents-edit-staff.php">
...
<option value="Y" <?php if($row['board'] == "Y"){ echo("SELECTED"); } ?>>Yes</option>
        ...
        <div class="row" style="padding: 10px 10px 10px 0px;">
            <div class="small-12 medium-12 columns"><strong>5) Ready to save your changes?</strong></div>
        </div>
...
<form action="documents-staff.php" method="get">
            <div class="small-6 columns" align="center">
...
<div class="small-12 medium-12 large-12 columns note-black"><br><br>Documents Edit (Staff) Control Panel Page<br></div>
</body>
</html>
