...

{
	...
	$query  = "SELECT * FROM packages WHERE pickedup > (NOW() - INTERVAL 32 DAY) LIMIT 1";

	$result = mysqli_query($conn,$query) or die('Error, query failed');
	list($int1, $recipient, $userid, $packagetype, $pkgtracking, $from, $pkglocation, $received, $recemp, $pickedup, $puemp, $pickedupby, $lastedited, $created, $comments) = 
		$result->fetch_array();

header("Content-Type: text/csv");
header("Content-Disposition: attachment; filename=Packages-31Days.csv");
}
?>
...
<?php
	$query  = "SELECT * FROM packages WHERE pickedup > (NOW() - INTERVAL 32 DAY) ORDER BY `int1`";
